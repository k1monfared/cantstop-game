...
function App() {
  ...
  // Player can roll if:
  // - It's their turn AND game not over AND not animating
  // - AND either (no dice showing) OR (has chosen a pairing and moved temp pegs)
  ...
  // Player can stop if:
  // - It's their turn AND has temp progress AND not bust AND not animating
  // - AND either (no dice showing) OR (has chosen a pairing)
  ...
  // BUT: Don't show preview if a pairing has already been chosen (pegs already moved)
  const activePairingIndex = hoveredPairing !== null ? hoveredPairing :
                             (needsNumberChoice && selectedPairing !== null ? selectedPairing : null)

  if (activePairingIndex !== null && !hasChosenPairing) {
